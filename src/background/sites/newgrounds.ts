...
const newgrounds_info: SiteInfo = {
    site: 'newgrounds',
    label: 'Newgrounds',
    links: {
...
const newgrounds_form = {
    ...
    fileArtMultiple: {
        ...
        default:
            'Saved/{site}/{userId}/art/art;{userId};{slug}_{submissionId}/art;{userId};{slug}_{submissionId}_{page}.{ext}',
...
type NewgroundsOptionsValues = SiteOptions<typeof newgrounds_form>;

type NewgroundsFileMeta = Pick<NewgroundsFile, 'fileName' | 'ext'>;
type NewgroundsFileData = FileData<NewgroundsFileMeta>;
type NewgroundsSubmissionMeta = Omit<NewgroundsFile, keyof NewgroundsFileMeta>;
type NewgroundsSubmissionData = SubmissionData<NewgroundsSubmissionMeta>;

type NewgroundsArtSubmissionMeta = Omit<NewgroundsArtFile, keyof NewgroundsFileMeta>;
type NewgroundsArtSubmissionData = SubmissionData<NewgroundsArtSubmissionMeta>;

type NewgroundsProjectType = 'game' | 'movie' | 'audio' | 'art';
type NewgroundsGameData = { file: string; info: FileInfo };
