function cleanWriting(element: HTMLElement, remove_attrs?: string[]) {
...
function doubleBrToP(element: HTMLElement) {
    ...
    while (child) {
        ...
        const has_breaks = Boolean((child as Element).querySelector?.('br'));
        if (block_node_names.includes(type) || has_breaks) {
            addP();
            if (child.nodeName === 'P') {
                div.append(...doubleBrToP(child as HTMLElement).childNodes);
            } else if (has_breaks) {
                div.append(doubleBrToP(child as HTMLElement));
            } else {
                div.append(child);
            }
            child = next;
