class VirtualList<T> {
    parent: HTMLElement;
    createRow: (v: T) => HTMLElement;
    values: T[];
    container: HTMLElement;
    viewport_height: number;
    rows = new Map<number, HTMLElement>();
    row_height = 18; //px
    default_height = 180; //px
    wait = false;

    constructor(parent: HTMLElement, createRow: (v: T) => HTMLElement, values?: T[]) {
...
class SearchList {
    values: string[] = [];
    virtual_list: VirtualList<VirtualSearchResult>;
    ...
    constructor(parent: HTMLElement, createRow: (v: VirtualSearchResult) => HTMLElement) {
        const search_input = parent.querySelector<HTMLInputElement>('[data-search]');
        ...
        const list_box = parent.querySelector<HTMLElement>('[data-list]');
        if (!list_box) {
        ...
        const clear_button = parent.querySelector<HTMLButtonElement>('[data-clear]');
        ...
        const case_flag = parent.querySelector<HTMLInputElement>('[data-match-case]');
        ...
        const whole_flag = parent.querySelector<HTMLInputElement>('[data-match-whole]');
        ...
        const regex_flag = parent.querySelector<HTMLInputElement>('[data-use-regex]');
        ...
        const sort_button = parent.querySelector<HTMLButtonElement>('[data-sort]');
    ...
    setVirtualList() {
        ...
        const result_values = searchValues(
            this.search_input?.value ?? '',
            this.values,
            match_case,
            match_whole,
    ...
    updateValues(new_values: (string | number)[]) {
...
function toggleListSort(sort_button: HTMLElement) {
...
function searchValues(
    ...
    let results: VirtualSearchResult[] = [];
    if (search) {
        if (use_regex) {
